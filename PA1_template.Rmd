...
```
The mean for the daily number of steps is `r mean`   and the median is `r median`. Format function was used in the previous code to make the value of the mean display nicely here.
...
The maximum average number of steps is in the interval `r max_interval`.
...
```
There are `r num_na` observations with no data.  
...
```  
The mean is `r mean2` and the median is `r median2`. There's no difference between the means, but the median is smaller than what was calculated earlier. The histograms looks different also.  
